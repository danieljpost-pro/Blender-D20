...
4. **Stages are independently runnable.** Scene-build, physics-bake, and
   per-outcome render can each be skipped if their inputs are unchanged.
   Don't break this.
...
gets back an MP4 of a die rolling and landing on that number.
...
4. Render. The physics cache is unchanged across all 20 possible outcomes —
   only the text on the face labels differs.
...
├── render.py        # Render-engine settings + preview modes.
...
# Features
--bowl                # enable hemispherical bowl
--bowl-radius R       # inner radius in meters
--bowl-depth D        # depth in meters (rim to bottom)
--no-bumpers
--no-dof
--no-rim-light
--no-fill-light
...
```bash
# Cheap iteration
make preview                         # Eevee, 25%, 8 samples
...
| Bowl instead of table | `BowlConfig` enabled + `table.visible=False` | costly | physics + render |
